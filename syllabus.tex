...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/env.png}
...
\begin{itemize}
\item the \emph{geth} console is a javascript REPL, any valid Javascript code is accepted. The commands below are actually \emph{web3} API calls, for all available commands see \cite{web3_api}
...
\subsubsection{Typical values to use}
\begin{itemize}
...
The contract template can be downloaded from: \url{https://raw.githubusercontent.com/fecjanky/iot_sc_tutorial/master/src/NumberGuessing_template.sol}
 ...
 One can use the \emph{Remix IDE} for developing smart contracts that includes the solidity compiler and also a test network to deploy and interact with the contract. It is available under \url{http://remix.ethereum.org/}
